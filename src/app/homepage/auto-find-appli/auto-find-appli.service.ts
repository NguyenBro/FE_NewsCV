...
export class AutoFindAppliService {
  ...
  public AutoJob = new AutoJob();
  public listAutoJob: AutoJob[] = [];
  ...
  public getListOfCompetences() {
    this.initListPool();
    return of(this.listAutoJob);
  }

  async initListPool() {
    this.getListAutoJob().subscribe((res) => {
      this.listAutoJob = res.data;
      console.log('listAutoJob', res.data);
    });
  }
  setJobByAutoJob(code?: string) {
  ...
  getListAutoJob() {
    ...
    return this.http.get<ResponseObject>(
      `${this.urlPath + '/api/v1/auto-job/get-all'}`,
      { headers: headers }
    );
  }
  ...
  deleteAutoJobByCode(code: string) {
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders({
